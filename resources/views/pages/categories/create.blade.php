                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-5">
                            ...
                            <select
                                class="form-control category-status-select{{ $errors->has('status') ? ' is-invalid' : '' }}"
                                id="status" name="status" required>
                                <option disabled selected value>Select Status</option>
                                @foreach($statuses as $status)
                                <option value="{{ $status }}" @if(isset($category) && $category->status == $status)
                                    selected @elseif(!isset($category) && $status == 'Active') selected @else
                                    @endif>{{ $status }}
                                </option>
                                @endforeach
                            </select>
